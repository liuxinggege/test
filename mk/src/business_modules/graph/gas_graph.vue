...
<div class="graph_content">
		...
		<gis-map  :addDeviceList="TypeList" @edit="edit"  ref="GisMap" :dragList="dragList"  v-if="$store.state.center.length"></gis-map>
	<!-- 模拟量-->
	<div v-if="editAnalog">
           ...
           //保存拖动
			...
			editAdd(ob,point){
				...
				if(ob.id == 72||ob.id == 102||ob.id == 103||ob.id == 104){//电源箱 传输接口 设备 电缆 交换机
					...
					this.nowGas.x3_point = point.x;
					this.nowGas.y3_point = point.y;
					this.editAnalog = true;
				}else if(ob.pid == this.state['sensorConfig']['switch'] ){//开关量
					this.editText = '添加' + ob.v;
					this.swit = {};
					this.swit.sensor_type = ob.id;
					this.swit.x3_point = point.x;
					this.swit.y3_point = point.y;
					this.editSwitchs = true;
				}
			},
