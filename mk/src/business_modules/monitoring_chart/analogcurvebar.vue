			...
			return {	
                tooltip: {
                    ...
                    formatter: function(params){ 
                        ...
                        list.push(
                            `
                                最大值：${vm.maxValue}${unit}<br/>
                                最小值：${vm.minValue}${unit}<br/>
                                平均值：${vm.avgValue}${unit}<br/>
...
//			    },
                ...
                yAxis:[
                        {
                            type: 'value',
                            name:vm.sensor.type,
                            max:vm.sensor.max || vm.valueRange[vm.sensor.sensor_type],
                            axisLabel:{formatter:'{value}' + vm.sensor.sensorUnit,},
                            nameGap:50,
                        ...
                        { 
                            ...
                            nameGap:50,
                            offset:60,
                            show:vm.feedDataList.length>0?true:false,
                            position: 'right',
                            splitLine: {show: false},
                            nameTextStyle:{fontWeight:'bold',fontSize:14},
                            axisLabel:{
            ...
            series: [ 
                {
                    ...
                    itemStyle: {normal: {color: this.rdata.realvalue|| '#409eff'}},
                    markPoint: {
                        data: [
                            {
                                type: 'min',
                ...
                }, 
                {
                    ...
                    animation:false,
                    itemStyle: {normal: {color:this.rdata.feedvalue||'#4215c5'}},   
                },
                {
			        name:'时间范围',
			        type:'line', 
	...
	methods: {
		...
		getinfo() {
                var me = this
                me.myChart = this.echarts.getInstanceByDom(document.getElementById('time'));
                if (me.myChart == undefined) {  
                me.myChart = this.echarts.init(document.getElementById('time'));
                }
