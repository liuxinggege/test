...
remote_dist=/root/server/domain-guard
res=res
...
./public/views/${res} ./public/views/redirect.ejs \
${static}/${md5_res} ${static}/clipboard.min.js ${static}/jquery.js ${static}/simJQ.js ${static}/qrcode.min.js \
${static}/html/
...
echo ">scp.."
remote=121.201.122.17
dist=${remote_dist}
ssh root@${remote} "cd $dist "
scp $tarname root@${remote}:${dist}
rm $tarname
ssh root@${remote} "cd $dist && tar zxf $tarname && rm -f $tarname && chown -R nobody.nogroup $dist && pm2 start domain-guard"
