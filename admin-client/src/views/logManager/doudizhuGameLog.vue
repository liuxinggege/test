  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="斗地主游戏日志"></el-popover>
        <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        <span class="title">斗地主游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="doudizhuGameLog.doudizhuGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
      ...
      <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="doudizhuGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogDoudizhuVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{doudizhuGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="斗地主游戏玩家日志"></el-popover>
          <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="doudizhuGameLog.DoudizhuStageLog" border highlight-current-row style="width: 100%;">
          ...
          <el-table-column prop="isBanker" label="庄家" width="100" :formatter="isWinnerFormat" align="center"/>
          ...
          <el-table-column prop="userGameData.handCardsNum" label="手牌" min-width="150" align="center" :formatter="cardNumsFunc"/>
        </el-table>
      </el-dialog>
    </el-card>
  </div>
...
import { DoudizhuGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  doudizhuGameLog: DoudizhuGameLogState = this.$store.state.doudizhuGameLog; //表单数据
  dialogDoudizhuVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetDoudizhuGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.doudizhuGameLog.totalCount = 1;
    }
    if (this.doudizhuGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBDOUDIZHUUSERDETAIL", payload);
    this.dialogDoudizhuVisible = true;
  }
  // 牌型
  cardNumsFunc(row) {
    let tmp = row.userGameData.handCardsNum;
    let cardRunesStr = "";
    tmp.forEach(e => {
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetDoudizhuGameLogExcel", queryItem).then(ret => {
