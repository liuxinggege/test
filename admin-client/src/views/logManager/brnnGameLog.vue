  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="百人牛牛游戏日志">
        </el-popover>
        <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        <span class="title">百人牛牛游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="brnnGameLog.brnnGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
      ...
      <el-col class="toolbar2">
         <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="brnnGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogBrnnVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{brnnGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="百人牛牛游戏玩家日志">
          </el-popover>
          <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="brnnGameLog.BrnnStageLog" border highlight-current-row style="width: 100%;">
          ...
          <!-- <el-table-column prop="userGameData.allPosAfterTaxPoint" label="税后金" width="100" align="center"></el-table-column> -->
          ...
          <!-- <el-table-column prop="userGameData.score" label="分数" width="100" align="center"></el-table-column> -->
          ...
          <el-table-column prop="tax" label="税率" width="100" align="center"></el-table-column>
        </el-table>
      </el-dialog>
    </el-card>
  </div>
...
import { BrnnGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  brnnGameLog: BrnnGameLogState = this.$store.state.brnnGameLog; //表单数据
  dialogBrnnVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetBrnnGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.brnnGameLog.totalCount = 1;
    }
    if (this.brnnGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBBRNNUSERDETAIL", payload);
    this.dialogBrnnVisible = true;
  }
  //日期整形
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetBrnnGameLogExcel", queryItem).then(ret => {
