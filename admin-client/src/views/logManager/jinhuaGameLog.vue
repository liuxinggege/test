  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="金花游戏日志"></el-popover>
        <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        <span class="title">金花游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="jinhuaGameLog.jinhuaGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
      ...
      <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="jinhuaGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogJinhuaVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{jinhuaGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="金花游戏玩家日志"></el-popover>
          <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="jinhuaGameLog.JinhuaStageLog" border highlight-current-row style="width: 100%;">
          ...
          <el-table-column prop="userGameData.handCards" label="手牌" min-width="150" :formatter="cardRunes" align="center"/>
        </el-table>
      </el-dialog>
    </el-card>
  </div>
...
import { JinhuaGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  jinhuaGameLog: JinhuaGameLogState = this.$store.state.jinhuaGameLog; //表单数据
  dialogJinhuaVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetJinhuaGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.jinhuaGameLog.totalCount = 1;
    }
    if (this.jinhuaGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBJINHUAUSERDETAIL", payload);
    this.dialogJinhuaVisible = true;
  }
  //日期整形
  ...
  suitPointFormatter(suit, point) {
    ...
    pointString = A2KSymbols[point - 2];
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetJinhuaGameLogExcel", queryItem).then(ret => {
