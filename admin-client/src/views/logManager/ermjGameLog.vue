  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="二人麻将游戏日志"></el-popover>
        <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        <span class="title">二人麻将游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="ermjGameLog.ermjGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
      ...
      <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="ermjGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogErmjVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{ermjGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="二人麻将游戏玩家日志"></el-popover>
          <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="ermjGameLog.ErmjStageLog" border highlight-current-row style="width: 100%;">
          <el-table-column prop="pos" label="座位号" width="120" align="center"/>
          <el-table-column prop="uid" label="uid" width="100" align="center"/>
          <el-table-column prop="isRobot" label="是否机器人" width="100" :formatter="isRobot" align="center"/>
...
import { ErmjGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  ermjGameLog: ErmjGameLogState = this.$store.state.ermjGameLog; //表单数据
  dialogErmjVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetErmjGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.ermjGameLog.totalCount = 1;
    }
    if (this.ermjGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBERMJUSERDETAIL", payload);
    this.dialogErmjVisible = true;
  }
  //日期整形
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetErmjGameLog", queryItem).then(ret => {
