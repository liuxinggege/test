  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="龙虎斗游戏日志">
        </el-popover>
        <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        <span class="title">龙虎斗游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="longhuGameLog.longhuGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
        ...
        <el-table-column label="牌型" align="center">
          <el-table-column prop="gameData.cards.0" label="龙区" width="100" align="center" :formatter="getHandleCardOne"/>
          <el-table-column prop="gameData.cards.1" label="虎区" width="100" align="center" :formatter="getHandleCardTwo"/>
        </el-table-column>
      ...
      <!--工具条-->
       <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="longhuGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogLonghuVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{longhuGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="龙虎斗游戏玩家日志">
          </el-popover>
          <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="longhuGameLog.LonghuStageLog" border highlight-current-row style="width: 100%;">
          ...
          <el-table-column label="压分情况" align="center">
            <el-table-column prop="userGameData.0" label="龙区压分" width="100" align="center" />
            <el-table-column prop="userGameData.1" label="虎区压分" width="100" align="center" />
            <el-table-column prop="userGameData.2" label="和区压分" width="100" align="center" />
          </el-table-column>
        </el-table>
      </el-dialog>
    </el-card>
...
import { LonghuGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  longhuGameLog: LonghuGameLogState = this.$store.state.longhuGameLog; //表单数据
  dialogLonghuVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetLonghuGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.longhuGameLog.totalCount = 1;
    }
    if (this.longhuGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBLONGHUUSERDETAIL", payload);
    this.dialogLonghuVisible = true;
  }
  //日期整形
  ...
  getHandleCardOne(row) {
    let rune = row.gameData.cards[0]
    let suit=this.getSuit(rune);
    let point=this.getPoint(rune);
    let cardRunes = this.suitPointFormatter(suit,point)
    let cardRunesStr = cardRunes.suit+cardRunes.point
    return cardRunesStr
  }
  getHandleCardTwo(row) {
    let rune = row.gameData.cards[1]
    let suit=this.getSuit(rune);
    let point=this.getPoint(rune);
    let cardRunes = this.suitPointFormatter(suit,point)
    let cardRunesStr = cardRunes.suit+cardRunes.point
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetLonghuGameLogExcel", queryItem).then(ret => {
