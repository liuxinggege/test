<template>
  <div>
    ...
    <!--列表-->
    <el-table :data="recoverSection.recoverSectionData" border highlight-current-row style="width: 100%;" max-height="460">
      <el-table-column prop="logDate" label="时间" min-width="175" align="center" :formatter="timeFormat"/>
      <el-table-column prop="from" label="from" min-width="100" align="center"/>
      <el-table-column prop="to" label="to" min-width="100" align="center"/>
      <el-table-column prop="money" label="改变金币" min-width="100" align="center"/>
      <el-table-column prop="optUser" label="操作人" min-width="175" align="center"/>
      <el-table-column prop="optDiscription" label="操作详情" min-width="120" align="center"/>
    </el-table>
    <!--工具条-->
    <el-col class="toolbar4">
      <el-pagination layout="total,sizes,prev, pager, next,jumper" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-sizes="[10,20,30,50]" :page-size="count" :total="recoverSection.totalCount"></el-pagination>
    </el-col>
  </div>
</template>
...
import { RecoverSection } from "../../store/stateInterface";
...
export default class LoginLog extends Vue {
  ...
  /*inital data*/
  recoverSection: RecoverSection = this.$store.state.recoverSection; //表单数据
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 10;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetRecoverSectionLog", queryItem);
  }
  searchData() {
    this.page = 1;
    this.loadData();
  ...
  downloadExcel() {
    const downloadExcelCfg = [
      ...
      { title: "改变金币", field: "money", type: "string" },
    ...
    myDispatch(this.$store, "GetRecoverSectionLogExcel", queryItem).then(
      ret => {
