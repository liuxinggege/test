  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="德州扑克游戏日志"></el-popover>
        <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        <span class="title">德州扑克游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="dezhoupukeGameLog.dezhoupukeGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
     ...
     <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="dezhoupukeGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogDezhoupukeVisible" width="80%">
        <el-col class="toolbar1">
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="德州扑克游戏玩家日志"></el-popover>
          <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
          <span class="title">玩家日志({{dezhoupukeGameLog.gameId}})</span>
          <span class="title">
            公牌：
            <i v-for="(item,index) in theCommandCards" :key="index" class="cardItem">{{item.suit}}{{item.point}}</i>
          </span>
        </el-col>
        <!--列表-->
        <el-table :data="dezhoupukeGameLog.DezhoupukeStageLog" border highlight-current-row style="width: 100%;">
          ...
          <el-table-column prop="userGameData.isBanker" label="庄家" width="100" :formatter="isWinnerFormat" align="center"/>
          ...
          <el-table-column prop="userGameData.handPai" label="手牌" min-width="150" align="center" :formatter="getHandleCardOne"/>
        </el-table>
      </el-dialog>
    </el-card>
  </div>
...
import { DezhoupukeGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  dezhoupukeGameLog: DezhoupukeGameLogState = this.$store.state
    .dezhoupukeGameLog; //表单数据
  dialogDezhoupukeVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetDezhoupukeGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.dezhoupukeGameLog.totalCount = 1;
    }
    if (this.dezhoupukeGameLog.next) {
  ...
  getHandleCardOne(row) {
    let tmp = row.userGameData.handCards;
    console.log(tmp);
    let cardRunesStr = "";
    tmp.forEach(e => {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBDEZHOUPUKEUSERDETAIL", payload);
    let tempArr: any = [];
    this.dezhoupukeGameLog.commandCards.forEach(item => {
      let suit = this.getSuit(item);
      let point = this.getPoint(item);
      let line: any = this.suitPointFormatter(suit, point);
      tempArr.push(line);
    });
    this.theCommandCards = tempArr;
    this.dialogDezhoupukeVisible = true;
  }
  ...
  isWinnerFormat(row, column) {
    return row.userGameData ? (row.userGameData.isDealer ? "是" : "否") : "否";
  }
  getHandleCard(rune) {
    if (rune === 1000) {
      return "小王";
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetDezhoupukeGameLogExcel", queryItem).then(
      ret => {
