  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="梭哈游戏日志">
        </el-popover>
        <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        <span class="title">梭哈游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="suohaGameLog.suohaGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
        ...
        <el-table-column prop="gameData.stageName" label="场次名" width="100" align="center" />
      ...
      <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="suohaGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogSuohaVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{suohaGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="梭哈游戏玩家日志">
          </el-popover>
          <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="suohaGameLog.SuohaStageLog" border highlight-current-row style="width: 100%;">
          ...
          <el-table-column prop="userGameData.rawWinFromTablePoints" label="初始桌面获取金币" width="150" align="center" />
          <el-table-column prop="userGameData.postTaxWinFromTablePoints" label="之后桌面获取金币" min-width="150" align="center" />
          ...
          <el-table-column prop="userGameData.finRound" label="结束轮数" min-width="150" align="center" />
          <el-table-column prop="userGameData.cardRunes" label="手牌" min-width="150" :formatter="cardRunes" align="center" />
        </el-table>
      </el-dialog>
    </el-card>
  </div>
...
import { SuohaGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  suohaGameLog: SuohaGameLogState = this.$store.state.suohaGameLog; //表单数据
  dialogSuohaVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetSuohaGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.suohaGameLog.totalCount = 1;
    }
    if (this.suohaGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBSUOHAUSERDETAIL", payload);
    this.dialogSuohaVisible = true;
  }
  //日期整形
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetSuohaGameLogExcel", queryItem).then(ret => {
