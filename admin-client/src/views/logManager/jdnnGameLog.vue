  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="经典牛牛游戏日志"></el-popover>
        <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        <span class="title">经典牛牛游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="jdnnGameLog.jdnnGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
      ...
      <!--工具条-->
       <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="jdnnGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogJdnnVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{jdnnGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="经典牛牛游戏玩家日志"></el-popover>
          <el-button v-popover:popover1 type="text" class="el-icon-info"></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="jdnnGameLog.JdnnStageLog" border highlight-current-row style="width: 100%;">
...
import { JdnnGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  jdnnGameLog: JdnnGameLogState = this.$store.state.jdnnGameLog; //表单数据
  dialogJdnnVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetJdnnGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.jdnnGameLog.totalCount = 1;
    }
    if (this.jdnnGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBJDNNUSERDETAIL", payload);
    this.dialogJdnnVisible = true;
  }
  //日期整形
  ...
  cardRunes(row, column) {
    let result = "";
    for (let element of row.userGameData.handCards) {
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetJdnnGameLogExcel", queryItem).then(ret => {
