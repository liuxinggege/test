  ...
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      <el-col class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="红黑游戏日志">
        </el-popover>
        <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        <span class="title">红黑游戏日志</span>
      </el-col>
      <!--工具条-->
      <div>
        <span>用户ID</span>
      ...
      <!--列表-->
      <el-table :data="hongheiGameLog.hongheiGameLogData" border highlight-current-row style="width: 100%;" max-height="460">
        ...
        <el-table-column label="牌型" align="center">
          <el-table-column prop="gameData.0" label="黑区" width="100" align="center" :formatter="gameDataFormat0"/>
          <el-table-column prop="gameData.1" label="红区" width="100" align="center" :formatter="gameDataFormat1"/>
        </el-table-column>
      ...
      <!--工具条-->
       <el-col class="toolbar2">
        <el-pagination ref="pageRef" layout="sizes, prev, next" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-count="hongheiGameLog.totalCount" :page-sizes="[10,20,30,50]"></el-pagination>
      </el-col>
      <!-- 详细信息 -->
      <el-dialog :visible.sync="dialogHongheiVisible" width="80%">
        <el-col class="toolbar1">
          <span class="title">玩家日志({{hongheiGameLog.gameId}})</span>
          <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="红黑游戏玩家日志">
          </el-popover>
          <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        </el-col>
        <!--列表-->
        <el-table :data="hongheiGameLog.HongheiStageLog" border highlight-current-row style="width: 100%;">
          ...
          <el-table-column label="压分情况" align="center">
            <el-table-column prop="userGameData.0" label="黑区压分" width="100" align="center" />
            <el-table-column prop="userGameData.1" label="红区压分" width="100" align="center" />
            <el-table-column prop="userGameData.2" label="幸运一击压分" width="100" align="center" />
          </el-table-column>
        </el-table>
      </el-dialog>
    </el-card>
...
import { HongheiGameLogState } from "../../store/stateInterface";
...
export default class GameLog extends Vue {
  ...
  /*inital data*/
  hongheiGameLog: HongheiGameLogState = this.$store.state.hongheiGameLog; //表单数据
  dialogHongheiVisible: boolean = false;
  userId: string = "";
  logTime: Date[] = [];
  page: number = 1; //当前页
  count: number = 30;
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetHongheiGameLog", queryItem).then(() => {
      this.setPagination();
    });
  }
  setPagination() {
    ...
    if (this.page === 1) {
      this.hongheiGameLog.totalCount = 1;
    }
    if (this.hongheiGameLog.next) {
  ...
  userDetail(index, row) {
    ...
    this.$store.commit("SET_CUBHONGHEIUSERDETAIL", payload);
    this.dialogHongheiVisible = true;
  }
  //日期整形
  ...
  //牌型
  gameDataFormat0(row){
    if(row.gameData.mapCards===undefined){
      return row.gameData[0]
    }else{
      return row.gameData.mapCards[0]
    }
  }
  gameDataFormat1(row){
    if(row.gameData.mapCards===undefined){
      return row.gameData[1]
    }else{
      return row.gameData.mapCards[1]
    }
  }
  //房间类型
  ...
  downloadExcel() {
    ...
    let queryItem: QueryItem = this.getQueryItem();
    myDispatch(this.$store, "GetHongheiGameLogExcel", queryItem).then(ret => {
