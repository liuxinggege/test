<template>
  <div class="dashboard-outer">
    <el-card class="dashboard-second">
      ...
      <!-- <span>接单状态</span>
      <el-select v-model="orderState" placeholder="请选择" style="width:110px; margin:20px 10px">
        <el-option v-for="item in jiedanArr" :key="item.value" :label="item.label" :value="item.value"></el-option>
      </el-select>-->
      <br>
      <span>注册时间</span>
      ...
      <!-- 列表 -->
      <el-table :data="businessUserData.data" border max-height="700" highlight-current-row style="width: 100%;font-size:10pt">
        ...
        <el-table-column prop="act" label="账号/手机" width="130" align="center" :formatter="formatterPhone"></el-table-column>
        <el-table-column prop="loginPhone" label="修改手机" width="200" align="center">
          <template slot-scope="scope">
            {{scope.row.loginPhone}}
            <el-button type="text" @click="fixPhone(scope.$index, scope.row)">
        ...
        <el-table-column label="转账" width="180" align="center">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="transferOut(scope.$index, scope.row)">转出</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="个人属性" width="80" align="center">
          <template slot-scope="scope">
            <el-button type="text" @click="attribution(scope.$index, scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--工具条-->
      <el-col class="toolbar2">
        <el-pagination layout="total,sizes,prev, pager, next,jumper" class="pag" @current-change="handleCurrentChange" @size-change="handleSizeChange" :current-page="page" :page-sizes="[10,20,30,50]" :page-size="count" :total="businessUserData.totalCount"></el-pagination>
      </el-col>
      <!-- fixPhone -->
      <el-dialog :visible.sync="fixPhoneFlg" width="500px" @close="close">
        <div style="margin:0px 0px 40px 125px;font-size:20px;">修改商人手机号码</div>
        <div style="margin:0px 0px 40px 125px">
          <span style="font-size:20px;">ID</span>
          <el-input v-model="businessUid" style="width:120px" disabled></el-input>
        </div>
        <div style="margin:0px 0px 40px 80px">
          <span style="font-size:20px;">新手机</span>
          <el-input v-model="phone" style="width:200px"></el-input>
          <br>
        </div>
        <el-button type="primary" @click="fixPhoneConfirm" style="margin:0px 0px 10px 230px">确认提交</el-button>
      </el-dialog>
      <!-- changeChannel -->
      ...
      <el-dialog :visible.sync="dialogMoneyRecoveryVisible" width="500px" @close="close">
        ...
        <div style="margin:0px 0px 40px 63px">
          ...
          <el-input :controls="false" v-model="raceRecoveryRMB" style="width:120px"></el-input>
          <br>
        </div>
        <br>
      ...
      <el-dialog :visible.sync="dialogTableVisible" width="600px">
        <div style="border:1px  solid #7FFFD4">
          ...
          <div style="margin:0px 0px 10px 100px">
            ...
            <span style="margin:0px 0 10px 20px">聚商</span>
          ...
          <el-button type="primary" @click="saveBusiness" style="margin:0px 0px 10px 100px">确认修改</el-button>
          <div style="margin:0px 0px 10px 100px">
            <span>权重</span>
...
export default class BusinessUserInfo extends Vue {
  ...
  businessUserData = this.businessUser.businessUserData;
  uid: string = "";
  act: string = "";
  pidList: any[] = [];
  pid: string = "A";
  ...
  fixPhone(index, row) {
    this.businessUid = row.uid;
    this.phone = "";
    this.fixPhoneFlg = true;
  }
  fixPhoneConfirm() {
    if (!this.phone) {
      this.$message({
    ...
    let tmp = {
      phone: this.phone,
      uid: this.businessUid
    };
    myDispatch(this.$store, "FixBusinessPhone", tmp).then(() => {
      if (this.businessUser.updateUserInfoCode === 200) {
        ...
        this.loadData();
        this.fixPhoneFlg = false;
      } else {
        this.$message({
          showClose: true,
          type: "error",
          message: `设置失败!${this.businessUser.msg}`
        });
      }
    });
  }
  ...
  loadData() {
    ...
    myDispatch(this.$store, "GetBusinessInfo", queryItem, true).then(() => {
      this.businessUserData = this.businessUser.businessUserData;
    });
  ...
  getQueryItem() {
    ...
    if (this.uid.trim()) {
      temp.uid = this.uid.trim();
    }
    if (this.channel.trim()) {
      temp.channel = this.channel.trim();
      if (this.channel === "官方") {
        temp.channel = "";
      }
    }
    if (this.act.trim()) {
      temp.act = this.act.trim();
    }
    if (this.name.trim()) {
      temp.name = this.name.trim();
    }
  ...
  withDrawConfirm() {
    ...
    if (parseInt(this.withDrawMoney) > 0 && parseInt(this.withDrawRMB) > 0) {
      //提示确认数据操作
      let RMB = this.numToChi(this.withDrawMoney);
      this.$confirm(
    ...
    this.$confirm("是否确认提交?", "提示", {
      ...
      .then(() => {
        ...
        myDispatch(this.$store, "UpdateBusinessInfo", payload).then(() => {
          ...
          if (this.businessUser.updateUserInfoCode == 1013) {
            this.$message({
              showClose: true,
              type: "error",
              message: "昵称重复，请更换"
