<template>
    <el-card class="dashboard-second">
        ...
        <el-table :data="matchStages.matchStagesData" border highlight-current-row style="width: 100%;margin: 20px 0">
            ...
            <el-table-column prop="taxRate" label="税率" width="65" align="center"></el-table-column>
            ...
            <el-table-column prop="ratio" label="房间倍率" width="65" align="center"></el-table-column>
            ...
            <el-table-column prop="ratePeriodG" label="高概率卡" width="130" align="center"></el-table-column>
            <el-table-column prop="ratePeriodD" label="低概率卡" width="150" align="center"></el-table-column>
            <el-table-column prop="ratePeriodTimeMin" label="计算概率卡最小时间" width="130" align="center"></el-table-column>
            <el-table-column prop="ratePeriodTimeMax" label="计算概率卡最高时间" width="150" align="center"></el-table-column>
        ...
        <!-- 水位区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width=800px title="捕鱼水位线" @open="open" @close="close">
          <Buyu-RobotWinRate 
        ...
        <el-dialog :visible.sync="dialogBuyuMatchStagesVisible"  title="捕鱼">
            <div>               
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="激活" border
                  v-model="BuyuMatchStages.active">
                </el-checkbox>
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="机器人开关" border
                  v-model="BuyuMatchStages.robotActive">
                </el-checkbox>
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">idx</span>
                <el-input v-model="BuyuMatchStages.idx" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="BuyuMatchStages.color" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">底分</span>
                <el-input v-model="BuyuMatchStages.bets" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">税率</span>
                <el-input v-model="BuyuMatchStages.taxRate" style="width:90px; margin:0 20px"></el-input>
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="BuyuMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="BuyuMatchStages.maxMoney" style="width:90px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="BuyuMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="BuyuMatchStages.robotMinMoney" style="width:90px ; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">概率牌的最小时间</span>
                <el-input v-model="BuyuMatchStages.ratePeriodTimeMin" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">概率牌的最大时间</span>
                <el-input v-model="BuyuMatchStages.ratePeriodTimeMax" style="width:90px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">高概率卡</span>
                <el-input v-model="ratePeriodGStr" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">低概率卡</span>
                <el-input v-model="ratePeriodDStr" style="width:120px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
...
import { BuyuMatchStagesState } from "../../../store/stateInterface";
...
@Component({
  components: {
    "Buyu-RobotWinRate": RobotWinRate
  }
})
export default class BuyuMatchStages extends Vue {
  // lifecycle hook
  created() {
    this.loadData();
  }
  gid = "BY"
  /*inital data*/
  matchStages: BuyuMatchStagesState = this.$store.state.buyuMatchStages; //表单数据
  dialogBuyuMatchStagesVisible: boolean = false; // 操作页面
  ...
  /*method*/
  loadData() {
     myDispatch(this.$store, "GetBuyuMatchStages", {}, true)
    // this.$store.dispatch("GetBuyuMatchStages")
      .then(()=>{
  ...
  robotActiveFormat(row){
   ...
   //全部编辑
    edit(index, row) {
         this.dialogBuyuMatchStagesVisible = true;
         this.BuyuMatchStages.id = row.id
         this.BuyuMatchStages.id = row.id
         this.BuyuMatchStages.active = row.active
         this.BuyuMatchStages.idx = row.idx
         this.BuyuMatchStages.color = row.color
         this.BuyuMatchStages.bets = row.bets
         this.BuyuMatchStages.ratio = row.ratio
         this.BuyuMatchStages.maxMoney = row.maxMoney
         this.BuyuMatchStages.minMoney = row.minMoney
         this.BuyuMatchStages.name = row.name
         this.BuyuMatchStages.robotMaxMoney = row.robotMaxMoney
         this.BuyuMatchStages.robotMinMoney = row.robotMinMoney
         this.BuyuMatchStages.ratePeriodTimeMin = row.ratePeriodTimeMin
         this.BuyuMatchStages.ratePeriodTimeMax = row.ratePeriodTimeMax
         this.BuyuMatchStages.taxRate = row.taxRate
         this.BuyuMatchStages.robotActive=row.robotActive
         this.ratePeriodGStr = row.ratePeriodG.join(",")
         this.ratePeriodDStr= row.ratePeriodD.join(",")
    ...
    confirmStages(){
      this.$confirm("是否确认保存?", "提示", {
        ...
        this.BuyuMatchStages.ratePeriodG = this.ratePeriodGStr.split(",")
        this.BuyuMatchStages.ratePeriodD = this.ratePeriodDStr.split(",")
        myDispatch(this.$store, "UpdateBuyuMatchStages",this.BuyuMatchStages)
        // this.$store.dispatch("UpdateBuyuMatchStages",this.BuyuMatchStages)
        .then(() => {
        if (this.matchStages.code === 200) {
          ...
          this.dialogBuyuMatchStagesVisible = false;
          this.loadData()
          return;
        } else {
          this.$message({
