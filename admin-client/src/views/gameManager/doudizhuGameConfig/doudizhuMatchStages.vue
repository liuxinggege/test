<template>
    <el-card class="dashboard-second">
		<el-popover ref="popover1" placement="top-start" itle="标题" width="200" trigger="hover" content="斗地主匹配房场次">
		</el-popover>
        ...
        <el-table :data="matchStages.matchStagesData" border highlight-current-row style="width: 100%;margin: 20px 0">
            ...
            <el-table-column prop="maxBet" label="最大赌注" width="130" align="center"></el-table-column>
        ...
        <!-- 水位区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width=800px title="斗地主水位线" @open="open" @close="close">
          <Doudizhu-RobotWinRate 
        ...
        <el-dialog :visible.sync="dialogDoudizhuMatchStagesVisible"  title="斗地主">
            <div>               
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="激活" border
                  v-model="DoudizhuMatchStages.active">
                </el-checkbox>
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="机器人开关" border
                  v-model="DoudizhuMatchStages.robotActive">
                </el-checkbox> 
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">场次名</span>
                <el-input v-model="DoudizhuMatchStages.name" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">房间ID</span>
                <el-input v-model="DoudizhuMatchStages.id" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">idx</span>
                <el-input v-model="DoudizhuMatchStages.idx" style="width:90px; margin:0 20px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="DoudizhuMatchStages.color" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">赌注</span>
                <el-input v-model="DoudizhuMatchStages.bets" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">最大赌注</span>
                <el-input v-model="DoudizhuMatchStages.maxBet" style="width:120px; margin:0 10px"></el-input>
            </div>            
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="DoudizhuMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="DoudizhuMatchStages.maxMoney" style="width:90px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="DoudizhuMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="DoudizhuMatchStages.robotMinMoney" style="width:90px ; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
...
import { DoudizhuMatchStagesState } from "../../../store/stateInterface";
...
@Component({
  components: {
    "Doudizhu-RobotWinRate": RobotWinRate
  }
})
export default class DoudizhuMatchStages extends Vue {
  // lifecycle hook
  created() {
    this.loadData();
  }
  gid = "DDZ"
  /*inital data*/
  matchStages: DoudizhuMatchStagesState = this.$store.state.doudizhuMatchStages; //表单数据
  dialogDoudizhuMatchStagesVisible: boolean = false; // 操作页面
  ...
  /*method*/
  loadData() {
     myDispatch(this.$store, "GetDoudizhuMatchStages", {}, true)
    // this.$store.dispatch("GetDoudizhuMatchStages")
     .then(()=>{
  ...
  waterRange() {
   ...
   //全部编辑
    edit(index, row) {
        this.dialogDoudizhuMatchStagesVisible = true;
        this.DoudizhuMatchStages.active = row.active
        this.DoudizhuMatchStages.bets = row.bets
        this.DoudizhuMatchStages.color = row.color
        this.DoudizhuMatchStages.id = row.id
        this.DoudizhuMatchStages.idx = row.idx
        this.DoudizhuMatchStages.maxBet = row.maxBet
        this.DoudizhuMatchStages.maxMoney = row.maxMoney
        this.DoudizhuMatchStages.minMoney = row.minMoney
        this.DoudizhuMatchStages.name = row.name
        this.DoudizhuMatchStages.robotMaxMoney = row.robotMaxMoney
        this.DoudizhuMatchStages.robotMinMoney = row.robotMinMoney
        this.DoudizhuMatchStages.robotActive=row.robotActive;
    }
    ...
    confirmStages(){
      this.$confirm("是否确认保存?", "提示", {
        ...
        myDispatch(this.$store, "UpdateDoudizhuMatchStages",this.DoudizhuMatchStages)
        // this.$store.dispatch("UpdateDoudizhuMatchStages",this.DoudizhuMatchStages)
        .then(() => {
        if (this.matchStages.code === 200) {
          ...
          this.dialogDoudizhuMatchStagesVisible = false;
          this.loadData()
          return;
        } else {
          this.$message({
