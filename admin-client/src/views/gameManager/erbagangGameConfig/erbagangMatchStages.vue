<template>
    <el-card class="dashboard-second">
        <el-popover ref="popover1" placement="top-start" itle="标题" width="200" trigger="hover" content="二八杠匹配房场次"></el-popover>
        ...
        <el-table :data="matchStages.matchStagesData" border highlight-current-row style="width: 100%;margin: 20px 0">
            ...
            <el-table-column prop="bankerMinMoney" label="上庄金额" width="100" align="center"></el-table-column>
            <el-table-column prop="bankerMaxCnt" label="连庄次数" width="100" align="center"></el-table-column>
            <el-table-column prop="maxAllBetPoint" label="最大下注金额" width="100" align="center"></el-table-column>
            <el-table-column prop="readyBankerMaxLen" label="上庄列表长度" width="100" align="center"></el-table-column>
            <el-table-column prop="robotMaxBetRealBanker" label="真实玩家坐庄时机器人总体最大下注" width="100" align="center"></el-table-column>
            <el-table-column prop="yardType" label="场次类型" width="100" align="center"></el-table-column>
            <el-table-column prop="fanMaxBoost" label="最大反奖" width="100" align="center"></el-table-column>
        ...
        <!-- 水位区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width="800px" title="二八杠水位线" @open="open" @close="close">
            <Erbagang-RobotWinRate v-bind:robotWinRateData="robotWinRate" :poolLine="poolLine" :gid="gid" :yid="yid" v-on:newPoolLine :change="newPoolLine_change"></Erbagang-RobotWinRate>
        </el-dialog>
        <el-dialog :visible.sync="dialogErbagangMatchStagesVisible" title="二八杠">
            <div>
                <el-checkbox type="text" style="margin:20px 13px" class="checkbox" label="激活" border v-model="ErbagangMatchStages.active"></el-checkbox>
                <el-checkbox type="text" style="margin:20px 13px" class="checkbox" label="机器人开关" border v-model="ErbagangMatchStages.robotActive"></el-checkbox>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">场次名</span>
                <el-input v-model="ErbagangMatchStages.name" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">房间ID</span>
                <el-input v-model="ErbagangMatchStages.id" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">idx</span>
                <el-input v-model="ErbagangMatchStages.idx" style="width:90px; margin:0 20px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="ErbagangMatchStages.color" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">赌注</span>
                <el-input v-model="ErbagangMatchStages.bets" style="width:90px; margin:0 20px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="ErbagangMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="ErbagangMatchStages.maxMoney" style="width:90px; margin:0 10px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="ErbagangMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="ErbagangMatchStages.robotMinMoney" style="width:90px ; margin:0 10px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">上庄金额</span>
                <el-input v-model="ErbagangMatchStages.bankerMinMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">连庄次数</span>
                <el-input v-model="ErbagangMatchStages.bankerMaxCnt" style="width:90px ; margin:0 10px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">最大下注金额</span>
                <el-input v-model="ErbagangMatchStages.maxAllBetPoint" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">上庄列表长度</span>
                <el-input v-model="ErbagangMatchStages.readyBankerMaxLen" style="width:90px ; margin:0 10px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">真实玩家坐庄时机器人总体最大下注</span>
                <el-input v-model="ErbagangMatchStages.robotMaxBetRealBanker" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">场次类型</span>
                <el-input v-model="ErbagangMatchStages.yardType" style="width:90px ; margin:0 10px"></el-input>
                <span style="font-size:15px;">最大返奖</span>
                <el-input v-model="ErbagangMatchStages.fanMaxBoost" style="width:90px ; margin:0 10px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
...
import { ErbagangMatchStagesState } from "../../../store/stateInterface";
...
@Component({
    components: {
        "Erbagang-RobotWinRate": RobotWinRate
    }
})
export default class ErbagangMatchStages extends Vue {
    // lifecycle hook
    created() {
        this.loadData();
    }
    gid = "EBG"
    /*inital data*/
    matchStages: ErbagangMatchStagesState = this.$store.state.erbagangMatchStages; //表单数据
    dialogErbagangMatchStagesVisible: boolean = false; // 操作页面
    ...
    /*method*/
    loadData() {
        myDispatch(this.$store, "GetErbagangMatchStages", {}, true)
            // this.$store.dispatch("GetErbagangMatchStages")
            .then(() => {
    ...
    //全部编辑
    edit(index, row) {
        this.dialogErbagangMatchStagesVisible = true;
        this.ErbagangMatchStages.active = row.active
        this.ErbagangMatchStages.bets = row.bets
        this.ErbagangMatchStages.color = row.color
        this.ErbagangMatchStages.id = row.id
        this.ErbagangMatchStages.idx = row.idx
        this.ErbagangMatchStages.maxMoney = row.maxMoney
        this.ErbagangMatchStages.minMoney = row.minMoney
        this.ErbagangMatchStages.name = row.name
        this.ErbagangMatchStages.robotMaxMoney = row.robotMaxMoney
        this.ErbagangMatchStages.robotMinMoney = row.robotMinMoney
        this.ErbagangMatchStages.bankerMinMoney = row.bankerMinMoney
        this.ErbagangMatchStages.bankerMaxCnt = row.bankerMaxCnt
        this.ErbagangMatchStages.maxAllBetPoint = row.maxAllBetPoint
        this.ErbagangMatchStages.readyBankerMaxLen = row.readyBankerMaxLen
        this.ErbagangMatchStages.yardType = row.yardType
        this.ErbagangMatchStages.robotMaxBetRealBanker = row.robotMaxBetRealBanker
        this.ErbagangMatchStages.fanMaxBoost = row.fanMaxBoost
        this.ErbagangMatchStages.robotActive = row.robotActive
    }
    ...
    confirmStages() {
        this.$confirm("是否确认保存?", "提示", {
            ...
            let query: any = { ...this.ErbagangMatchStages };
            myDispatch(this.$store, "UpdateErbagangMatchStages", query)
                // this.$store.dispatch("UpdateErbagangMatchStages",this.ErbagangMatchStages)
                .then(() => {
                    if (this.matchStages.code === 200) {
                        ...
                        this.dialogErbagangMatchStagesVisible = false;
                        this.loadData()
                        return;
                    } else {
                        this.$message({
