<template>
    <el-card class="dashboard-second">
		<el-popover ref="popover1" placement="top-start" itle="标题" width="200" trigger="hover" content="二人麻将匹配房场次">
		</el-popover>
        ...
        <!-- 水位区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width=800px title="二人麻将水位线" @open="open" @close="close">
          <Ermj-RobotWinRate 
        ...
        <el-dialog :visible.sync="dialogErmjMatchStagesVisible"  title="二人麻将">
            <div>               
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="激活" border
                  v-model="ErmjMatchStages.active">
                </el-checkbox>
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="机器人开关" border
                  v-model="ErmjMatchStages.robotActive">
                </el-checkbox> 
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">idx</span>
                <el-input v-model="ErmjMatchStages.idx" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="ErmjMatchStages.color" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">底分</span>
                <el-input v-model="ErmjMatchStages.bets" style="width:90px; margin:0 20px"></el-input>
               
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="ErmjMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="ErmjMatchStages.maxMoney" style="width:90px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="ErmjMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="ErmjMatchStages.robotMinMoney" style="width:90px ; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
...
import { ErmjMatchStagesState } from "../../../store/stateInterface";
...
@Component({
  components: {
    "Ermj-RobotWinRate": RobotWinRate
  }
})
export default class ErmjMatchStages extends Vue {
  // lifecycle hook
  created() {
    this.loadData();
  }
  gid = "ERMJ"
  /*inital data*/
  matchStages: ErmjMatchStagesState = this.$store.state.ermjMatchStages; //表单数据
  dialogErmjMatchStagesVisible: boolean = false; // 操作页面
  ...
  /*method*/
  loadData() {
     myDispatch(this.$store, "GetErmjMatchStages", {}, true)
    // this.$store.dispatch("GetErmjMatchStages")
    .then(()=>{
  ...
  robotActiveFormat(row){
   ...
   //全部编辑
    edit(index, row) {
         this.dialogErmjMatchStagesVisible = true;
         this.ErmjMatchStages.active = row.active
         this.ErmjMatchStages.bets = row.bets
         this.ErmjMatchStages.color = row.color
         this.ErmjMatchStages.id = row.id
         this.ErmjMatchStages.idx = row.idx
         this.ErmjMatchStages.maxMoney = row.maxMoney
         this.ErmjMatchStages.minMoney = row.minMoney
         this.ErmjMatchStages.minRobotRoomCount = row.minRobotRoomCount
         this.ErmjMatchStages.minRoomCount = row.minRoomCount
         this.ErmjMatchStages.name = row.name
         this.ErmjMatchStages.poolLine = row.poolLine
         this.ErmjMatchStages.poolValue = row.poolValue
         this.ErmjMatchStages.robotMaxMoney = row.robotMaxMoney
         this.ErmjMatchStages.robotMinMoney = row.robotMinMoney
         this.ErmjMatchStages.robotActive = row.robotActive
    ...
    confirmStages(){
      this.$confirm("是否确认保存?", "提示", {
        ...
        myDispatch(this.$store, "UpdateErmjMatchStages",this.ErmjMatchStages)
        // this.$store.dispatch("UpdateErmjMatchStages",this.ErmjMatchStages)
        .then(() => {
        if (this.matchStages.code === 200) {
          ...
          this.dialogErmjMatchStagesVisible = false;
          this.loadData()
          return;
        } else {
          this.$message({
