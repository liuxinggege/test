<template>
    <el-card class="dashboard-second">
		<el-popover ref="popover1" placement="top-start" itle="标题" width="200" trigger="hover" content="经典牛牛匹配房场次">
		</el-popover>
        ...
        <el-table :data="matchStages.matchStagesData" border highlight-current-row style="width: 100%;margin: 20px 0">
         ...
         <el-dialog :visible.sync="dialogJDNiuniuMatchStagesVisible"  title="经典牛牛">
            <div>               
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="激活" border
                  v-model="JDNiuniuMatchStages.active">
                </el-checkbox>
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="机器人开关" border
                  v-model="JDNiuniuMatchStages.robotActive">
                </el-checkbox>                
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">idx</span>
                <el-input v-model="JDNiuniuMatchStages.idx" style="width:90px; margin:0 10px"></el-input>
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="JDNiuniuMatchStages.color" style="width:90px; margin:0 10px"></el-input>
                <span style="font-size:15px;">底分</span>
                <el-input v-model="JDNiuniuMatchStages.bets" style="width:90px; margin:0 10px"></el-input>               
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="JDNiuniuMatchStages.maxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>                
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="JDNiuniuMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>                
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="JDNiuniuMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>                
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="JDNiuniuMatchStages.robotMinMoney" style="width:120px; margin:0 50px 0 10px"></el-input>           
            </div> 
            <div style="margin:0 13px 20px">
        ...
        <!-- 水位线区间视图 -->
         <!-- 水池配置，水位线区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width=800px title="经典牛牛水位线" @open="open" @close="close">
          <JDNiuniu-RobotWinRate 
...
import { JDNiuniuMatchStagesState } from "../../../store/stateInterface"; //state Interface
...
@Component({
  components: {
    "JDNiuniu-RobotWinRate": RobotWinRate
  }
})
export default class JDNiuniuMatchStages extends Vue {
  // lifecycle hook
  created() {
    this.loadData();
  }
  /*inital data*/
  gid="JDNN"
  matchStages: JDNiuniuMatchStagesState = this.$store.state.jdniuniuMatchStages; //表单数据
  dialogRobotWinRateVisible: boolean = false; //整个水位线视图状态
  dialogJDNiuniuMatchStagesVisible: boolean = false; //全部编辑视图状态
  JDNiuniuMatchStages = {//全部编辑内容
    active:true,
    robotActive:false,
    bets:"0",
    color:0,
  ...
  loadData() {
    myDispatch(this.$store, "GetJDNiuniuMatchStages", {}, true)
    // this.$store.dispatch("GetJDNiuniuMatchStages")
      .then(()=>{
  ...
  //表格内编辑--取消
   ...
   //全部编辑
    edit(index, row) {
      this.dialogJDNiuniuMatchStagesVisible = true;
      this.JDNiuniuMatchStages.active = row.active
      this.JDNiuniuMatchStages.bets = row.bets
      this.JDNiuniuMatchStages.color = row.color
      this.JDNiuniuMatchStages.id = row.id
      this.JDNiuniuMatchStages.idx = row.idx
      this.JDNiuniuMatchStages.maxMoney = row.maxMoney
      this.JDNiuniuMatchStages.minMoney = row.minMoney
      this.JDNiuniuMatchStages.name = row.name
      this.JDNiuniuMatchStages.robotMaxMoney = row.robotMaxMoney
      this.JDNiuniuMatchStages.robotMinMoney = row.robotMinMoney
      this.JDNiuniuMatchStages.robotActive=row.robotActive
    }
    confirmStages(){
      this.$confirm("是否确认保存?", "提示", {
        ...
        myDispatch(this.$store, "UpdateJDNiuniuMatchStages" ,this.JDNiuniuMatchStages)
        // this.$store.dispatch("UpdateJDNiuniuMatchStages",this.JDNiuniuMatchStages)
        .then(() => {
        if (this.matchStages.code === 200) {
          ...
          this.dialogJDNiuniuMatchStagesVisible = false;
          this.loadData()
          return;
        } else {
          this.$message({
