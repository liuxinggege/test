<template>
    <el-card class="dashboard-second">
		<el-popover ref="popover1" placement="top-start" itle="标题" width="200" trigger="hover" content="红黑匹配房场次">
		</el-popover>
        ...
        <el-table :data="matchStages.matchStagesData" border highlight-current-row style="width: 100%;margin: 20px 0">
            ...
            <el-table-column prop="blackOrRedMaxMoney" label="单区域最大下注" width="130" align="center"></el-table-column>
            <el-table-column prop="specialMaxMoney" label="特殊区域最大下注" width="150" align="center"></el-table-column>
            <el-table-column prop="robotSpecialMaxMoney" label="机器人特殊区域最大下注金额" width="130" align="center"></el-table-column>
            <el-table-column prop="allowBetMinMoney" label="最小允许剩余金币下注" width="150" align="center"></el-table-column>
            <el-table-column prop="roomBetMaxMoney" label="房间最大下注上限" width="150" align="center"></el-table-column>
        ...
        <!-- 水位区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width=800px title="红黑水位线" @open="open" @close="close">
          <Honghei-RobotWinRate 
        ...
        <el-dialog :visible.sync="dialogHongheiMatchStagesVisible"  title="红黑">
            <div>               
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="激活" border
                  v-model="HongheiMatchStages.active">
                </el-checkbox>
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="机器人开关" border
                  v-model="HongheiMatchStages.robotActive">
                </el-checkbox>
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">idx</span>
                <el-input v-model="HongheiMatchStages.idx" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="HongheiMatchStages.color" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">底分</span>
                <el-input v-model="HongheiMatchStages.bets" style="width:90px; margin:0 20px"></el-input>
               
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="HongheiMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="HongheiMatchStages.maxMoney" style="width:90px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="HongheiMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="HongheiMatchStages.robotMinMoney" style="width:90px ; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">单区域最大下注</span>
                <el-input v-model="HongheiMatchStages.blackOrRedMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">特殊区域最大下注</span>
                <el-input v-model="HongheiMatchStages.specialMaxMoney" style="width:90px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人特殊区域最大下注金额</span>
                <el-input v-model="HongheiMatchStages.robotSpecialMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">最小允许剩余金币下注</span>
                <el-input v-model="HongheiMatchStages.allowBetMinMoney" style="width:120px; margin:0 10px"></el-input>
                <span style="font-size:15px;">房间最大下注</span>
                <el-input v-model="HongheiMatchStages.roomBetMaxMoney" style="width:120px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
...
import { HongheiMatchStagesState } from "../../../store/stateInterface";
...
@Component({
  components: {
    "Honghei-RobotWinRate": RobotWinRate
  }
})
export default class HongheiMatchStages extends Vue {
  // lifecycle hook
  created() {
    this.loadData();
  }
  gid = "HH"
  /*inital data*/
  matchStages: HongheiMatchStagesState = this.$store.state.hongheiMatchStages; //表单数据
  dialogHongheiMatchStagesVisible: boolean = false; // 操作页面
  ...
  loadData() {
     myDispatch(this.$store, "GetHongheiMatchStages", {}, true)
    // this.$store.dispatch("GetHongheiMatchStages")
    .then(()=>{
  ...
  waterRange() {
   ...
   //全部编辑
    edit(index, row) {
         this.dialogHongheiMatchStagesVisible = true;
         this.HongheiMatchStages.active = row.active
         this.HongheiMatchStages.bets = row.bets
         this.HongheiMatchStages.color = row.color
         this.HongheiMatchStages.id = row.id
         this.HongheiMatchStages.idx = row.idx
         this.HongheiMatchStages.allowBetMinMoney = row.allowBetMinMoney
         this.HongheiMatchStages.blackOrRedMaxMoney = row.blackOrRedMaxMoney
         this.HongheiMatchStages.maxMoney = row.maxMoney
         this.HongheiMatchStages.maxRobotCount = row.maxRobotCount
         this.HongheiMatchStages.minMoney = row.minMoney
         this.HongheiMatchStages.minRobotRoomCount = row.minRobotRoomCount
         this.HongheiMatchStages.minRoomCount = row.minRoomCount
         this.HongheiMatchStages.name = row.name
         this.HongheiMatchStages.personBetMaxMoney = row.personBetMaxMoney
         this.HongheiMatchStages.poolLine = row.poolLine
         this.HongheiMatchStages.poolValue = row.poolValue
         this.HongheiMatchStages.robotMaxMoney = row.robotMaxMoney
         this.HongheiMatchStages.robotMinMoney = row.robotMinMoney
         this.HongheiMatchStages.robotSpecialMaxMoney = row.robotSpecialMaxMoney
         this.HongheiMatchStages.roomBetMaxMoney = row.roomBetMaxMoney
         this.HongheiMatchStages.specialMaxMoney = row.specialMaxMoney
         this.HongheiMatchStages.robotActive=row.robotActive
    ...
    confirmStages(){
      this.$confirm("是否确认保存?", "提示", {
        ...
        myDispatch(this.$store, "UpdateHongheiMatchStages",this.HongheiMatchStages)
        // this.$store.dispatch("UpdateHongheiMatchStages",this.HongheiMatchStages)
        .then(() => {
        if (this.matchStages.code === 200) {
          ...
          this.dialogHongheiMatchStagesVisible = false;
          this.loadData()
          return;
        } else {
          this.$message({
