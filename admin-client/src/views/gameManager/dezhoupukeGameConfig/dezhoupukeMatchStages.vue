<template>
    <el-card class="dashboard-second">
		<el-popover ref="popover1" placement="top-start" itle="标题" width="200" trigger="hover" content="德州扑克匹配房场次">
		</el-popover>
        ...
        <el-table :data="matchStages.matchStagesData" border highlight-current-row style="width: 100%;margin: 20px 0">
            ...
            <el-table-column prop="maxTakeMoney" label="进场携带金额" width="130" align="center"></el-table-column>
        ...
        <!-- 水位区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width=800px title="德州扑克水位线" @open="open" @close="close">
          <Dezhoupuke-RobotWinRate 
        ...
        <el-dialog :visible.sync="dialogDezhoupukeMatchStagesVisible"  title="德州扑克">
            <div>               
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="激活" border
                  v-model="DezhoupukeMatchStages.active">
                </el-checkbox>
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="机器人开关" border
                  v-model="DezhoupukeMatchStages.robotActive">
                </el-checkbox> 
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">场次名</span>
                <el-input v-model="DezhoupukeMatchStages.name" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">房间ID</span>
                <el-input v-model="DezhoupukeMatchStages.id" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">idx</span>
                <el-input v-model="DezhoupukeMatchStages.idx" style="width:90px; margin:0 20px"></el-input>
            </div>
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="DezhoupukeMatchStages.color" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">赌注</span>
                <el-input v-model="DezhoupukeMatchStages.bets" style="width:90px; margin:0 20px"></el-input>
                <span style="font-size:15px;">进场携带金额</span>
                <el-input v-model="DezhoupukeMatchStages.maxTakeMoney" style="width:120px; margin:0 10px"></el-input>
            </div>            
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="DezhoupukeMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="DezhoupukeMatchStages.maxMoney" style="width:90px; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="DezhoupukeMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="DezhoupukeMatchStages.robotMinMoney" style="width:90px ; margin:0 10px"></el-input>
            </div> 
            <div style="margin:0 13px 20px">
...
import { DezhoupukeMatchStagesState } from "../../../store/stateInterface";
...
@Component({
  components: {
    "Dezhoupuke-RobotWinRate": RobotWinRate
  }
})
export default class DezhoupukeMatchStages extends Vue {
  // lifecycle hook
  created() {
    this.loadData();
  }
  gid = "DZPK"
  /*inital data*/
  matchStages: DezhoupukeMatchStagesState = this.$store.state.dezhoupukeMatchStages; //表单数据
  dialogDezhoupukeMatchStagesVisible: boolean = false; // 操作页面
  ...
  /*method*/
  loadData() {
     myDispatch(this.$store, "GetDezhoupukeMatchStages", {}, true)
    // this.$store.dispatch("GetDezhoupukeMatchStages")
     .then(()=>{
  ...
  waterRange() {
   ...
   //全部编辑
    edit(index, row) {
        this.dialogDezhoupukeMatchStagesVisible = true;
        this.DezhoupukeMatchStages.active = row.active
        this.DezhoupukeMatchStages.bets = row.bets
        this.DezhoupukeMatchStages.color = row.color
        this.DezhoupukeMatchStages.id = row.id
        this.DezhoupukeMatchStages.idx = row.idx
        this.DezhoupukeMatchStages.maxTakeMoney = row.maxTakeMoney
        this.DezhoupukeMatchStages.maxMoney = row.maxMoney
        this.DezhoupukeMatchStages.minMoney = row.minMoney
        this.DezhoupukeMatchStages.name = row.name
        this.DezhoupukeMatchStages.robotMaxMoney = row.robotMaxMoney
        this.DezhoupukeMatchStages.robotMinMoney = row.robotMinMoney
        this.DezhoupukeMatchStages.robotActive=row.robotActive;
    }
    ...
    confirmStages(){
      this.$confirm("是否确认保存?", "提示", {
        ...
        myDispatch(this.$store, "UpdateDezhoupukeMatchStages",this.DezhoupukeMatchStages)
        // this.$store.dispatch("UpdateDezhoupukeMatchStages",this.DezhoupukeMatchStages)
        .then(() => {
        if (this.matchStages.code === 200) {
          ...
          this.dialogDezhoupukeMatchStagesVisible = false;
          this.loadData()
          return;
        } else {
          this.$message({
