<template>
    <el-card class="dashboard-second">
		<el-popover ref="popover1" placement="top-start" itle="标题" width="200" trigger="hover" content="牛牛匹配房场次">
		</el-popover>
        ...
        <el-table :data="matchStages.matchStagesData" border highlight-current-row style="width: 100%;margin: 20px 0">
         ...
         <el-dialog :visible.sync="dialogNiuniuMatchStagesVisible"  title="抢庄牛牛">
            <div>               
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="激活" border
                  v-model="NiuniuMatchStages.active">
                </el-checkbox>
                <el-checkbox type='text' style="margin:20px 13px" class="checkbox" label="机器人开关" border
                  v-model="NiuniuMatchStages.robotActive">
                </el-checkbox>                
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">场次名</span>
                <el-input v-model="NiuniuMatchStages.name" style="width:90px; margin:0 10px"></el-input>
                <span style="font-size:15px;">idx</span>
                <el-input v-model="NiuniuMatchStages.idx" style="width:90px; margin:0 10px"></el-input>
                <span style="font-size:15px;">颜色</span>
                <el-input v-model="NiuniuMatchStages.color" style="width:90px; margin:0 10px"></el-input>
                <span style="font-size:15px;">底分</span>
                <el-input v-model="NiuniuMatchStages.bets" style="width:90px; margin:0 10px"></el-input>    
            </div>           
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最大携带金币</span>
                <el-input v-model="NiuniuMatchStages.maxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>                
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">进房最小携带金币</span>
                <el-input v-model="NiuniuMatchStages.minMoney" style="width:120px;margin:0 50px 0 10px"></el-input>                
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最大金币数</span>
                <el-input v-model="NiuniuMatchStages.robotMaxMoney" style="width:120px;margin:0 50px 0 10px"></el-input>                
            </div> 
            <div style="margin:0 13px 20px">
                <span style="font-size:15px;">机器人最小金币数</span>
                <el-input v-model="NiuniuMatchStages.robotMinMoney" style="width:120px; margin:0 50px 0 10px"></el-input>           
            </div> 
            <div style="margin:0 13px 20px">
        ...
        <!-- 水位线区间视图 -->
         <!-- 水池配置，水位线区间 -->
        <el-dialog :visible.sync="dialogRobotWinRateVisible" width=800px title="抢庄牛牛水位线" @open="open" @close="close">
          <Niuniu-RobotWinRate 
...
import { NiuniuMatchStagesState } from "../../../store/stateInterface"; //state Interface
...
@Component({
  components: {
    "Niuniu-RobotWinRate": RobotWinRate
  }
})
export default class NiuniuMatchStages extends Vue {
  // lifecycle hook
  created() {
    this.loadData();
  }
  /*inital data*/
  gid="QZNN"
  matchStages: NiuniuMatchStagesState = this.$store.state.niuniuMatchStages; //表单数据
  dialogRobotWinRateVisible: boolean = false; //整个水位线视图状态
  dialogNiuniuMatchStagesVisible: boolean = false; //全部编辑视图状态
  NiuniuMatchStages = {//全部编辑内容
    active:true,
    robotActive:false,
    bets:"0",
    color:0,
  ...
  loadData() {
    myDispatch(this.$store, "GetNiuniuMatchStages", {}, true)
    // this.$store.dispatch("GetNiuniuMatchStages")
      .then(()=>{
  ...
  //表格内编辑--取消
   ...
   //全部编辑
    edit(index, row) {
      this.dialogNiuniuMatchStagesVisible = true;
      this.NiuniuMatchStages.active = row.active
      this.NiuniuMatchStages.bets = row.bets
      this.NiuniuMatchStages.robotActive = row.robotActive
      this.NiuniuMatchStages.color = row.color
      this.NiuniuMatchStages.id = row.id
      this.NiuniuMatchStages.idx = row.idx
      this.NiuniuMatchStages.maxMoney = row.maxMoney
      this.NiuniuMatchStages.minMoney = row.minMoney
      this.NiuniuMatchStages.name = row.name
      this.NiuniuMatchStages.robotMaxMoney = row.robotMaxMoney
      this.NiuniuMatchStages.robotMinMoney = row.robotMinMoney
    }
    confirmStages(){
      this.$confirm("是否确认保存?", "提示", {
        ...
        myDispatch(this.$store, "UpdateNiuniuMatchStages" ,this.NiuniuMatchStages)
        // this.$store.dispatch("UpdateNiuniuMatchStages",this.NiuniuMatchStages)
        .then(() => {
        if (this.matchStages.code === 200) {
          ...
          this.dialogNiuniuMatchStagesVisible = false;
          this.loadData()
          return;
        } else {
          this.$message({
