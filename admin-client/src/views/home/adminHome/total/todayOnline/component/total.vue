<template>
     <div>
        <div id="chartLine1" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  async loadData() {
    console.log(document.getElementById("chartLine1"))
    await myDispatch(this.$store, "GetTodayAndYestRealOnline",  null, true).then(() => {
      ...
      this.todayOnline.forEach(item => {
        ...
        yData1.push(Number(item.totalRealOnline));
      });
      this.chartLine1.showLoading({
        text: ""
      });

      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.totalRealOnline));
      });
