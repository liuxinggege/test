<template>
     <div>
        <div id="chartLine14" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLine14"))
    myDispatch(this.$store, "GetTodayAndYestRealOnline", {}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLine14 = echarts.init(document.getElementById("chartLine14"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.ermjRealOnline));
      });
      this.chartLine14.showLoading({
        text: ""
      });
