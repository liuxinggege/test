<template>
     <div>
        <div id="chartLine10" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLine10"))
    myDispatch(this.$store, "GetTodayAndYestRealOnline", null, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLine10 = echarts.init(document.getElementById("chartLine10"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.buyuRealOnline));
      });
      this.chartLine10.showLoading({
        text: ""
      });
