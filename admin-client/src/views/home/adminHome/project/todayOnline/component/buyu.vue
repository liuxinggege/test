<template>
     <div>
        <div id="chartLineYQ10" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLineYQ10"))
    myDispatch(this.$store, "GetPTodayAndYestRealOnline", {pid:this.pid}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ10 = echarts.init(document.getElementById("chartLineYQ10"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.buyuRealOnline));
      });
      this.chartLineYQ10.showLoading({
        text: ""
      });

      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.buyuRealOnline));
      });
  ...
  drawLineChart(xData, yData1, yData2) {
    ...
    this.chartLineYQ10.hideLoading();
  }
}
</script>
