<template>
     <div>
        <div id="chartLineYQ14" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLineYQ14"))
    myDispatch(this.$store, "GetTodayAndYestRealOnline", {pid:this.pid}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ14 = echarts.init(document.getElementById("chartLineYQ14"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.ermjRealOnline));
      });
      this.chartLineYQ14.showLoading({
        text: ""
      });

      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.ermjRealOnline));
      });
  ...
  drawLineChart(xData, yData1, yData2) {
    ...
    this.chartLineYQ14.hideLoading();
  }
}
</script>
