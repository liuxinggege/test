...
export default class Total extends Vue {
  ...
  loadData() {
    ...
    myDispatch(
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ14 = echarts.init(
        document.getElementById("chartLineYQ14")
      );

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.qianghongbaoRealOnline));
      });
      ...
      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.qianghongbaoRealOnline));
      });
