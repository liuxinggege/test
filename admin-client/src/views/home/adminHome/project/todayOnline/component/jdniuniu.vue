<template>
  <div>
    <div id="chartLineYQ11" style="width:100%; height:630px;padding: 10px;"></div>
  </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLineYQ11"))
    myDispatch(this.$store, "GetPTodayAndYestRealOnline",  {pid:this.pid}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ11 = echarts.init(document.getElementById("chartLineYQ11"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.jdniuniuRealOnline));
      });
      this.chartLineYQ11.showLoading({
        text: ""
      });

      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.jdniuniuRealOnline));
      });
  ...
  drawLineChart(xData, yData1, yData2) {
    ...
    this.chartLineYQ11.hideLoading();
  }
}
</script>
