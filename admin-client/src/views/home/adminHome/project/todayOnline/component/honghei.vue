<template>
     <div>
        <div id="chartLineYQ8" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLineYQ8"))
    myDispatch(this.$store, "GetPTodayAndYestRealOnline", {pid:this.pid}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ8 = echarts.init(document.getElementById("chartLineYQ8"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.hongheiRealOnline));
      });
      this.chartLineYQ8.showLoading({
        text: ""
      });
