<template>
     <div>
        <div id="chartLineYQ6" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLineYQ6"))
    myDispatch(this.$store, "GetPTodayAndYestRealOnline",  {pid:this.pid}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ6 = echarts.init(document.getElementById("chartLineYQ6"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.jinhuaRealOnline));
      });
      this.chartLineYQ6.showLoading({
        text: ""
      });

      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.jinhuaRealOnline));
      });
