<template>
  <div>
    <div id="chartLineYQ13" style="width:100%; height:630px;padding: 10px;"></div>
  </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLineYQ13"))
    myDispatch(this.$store, "GetPTodayAndYestRealOnline", {pid:this.pid}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ13 = echarts.init(document.getElementById("chartLineYQ13"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.dezhoupukeRealOnline));
      });
      this.chartLineYQ13.showLoading({
        text: ""
      });

      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.dezhoupukeRealOnline));
      });
  ...
  drawLineChart(xData, yData1, yData2) {
    ...
    this.chartLineYQ13.hideLoading();
  }
}
</script>
