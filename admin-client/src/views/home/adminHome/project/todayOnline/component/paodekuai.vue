<template>
     <div>
        <div id="chartLineYQ12" style="width:100%; height:630px;padding: 10px;"></div>
    </div>
</template>
...
export default class Total extends Vue {
  ...
  loadData() {
    console.log(document.getElementById("chartLineYQ12"))
    myDispatch(this.$store, "GetPTodayAndYestRealOnline",  {pid:this.pid}, true).then(() => {
      ...
      let yData2: number[] = [];
      //echarts.dispose()
      this.chartLineYQ12 = echarts.init(document.getElementById("chartLineYQ12"));

      this.todayOnline.forEach(item => {
        xData.push(item.graphDate);
        yData1.push(Number(item.paodekuaiRealOnline));
      });
      this.chartLineYQ12.showLoading({
        text: ""
      });

      this.yesterdayOnline.forEach(item => {
        yData2.push(Number(item.paodekuaiRealOnline));
      });
  ...
  drawLineChart(xData, yData1, yData2) {
    ...
    this.chartLineYQ12.hideLoading();
  }
}
</script>
