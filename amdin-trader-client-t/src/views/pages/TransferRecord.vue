  ...
  <div class="dashboard-editor-container">
    <el-card class="dashboard-second">
      <el-col  class="toolbar1">
        <el-popover ref="popover1" placement="top" itle="标题" trigger="hover" content="转入记录">
        </el-popover>
        <el-button v-popover:popover1 type='text' class='el-icon-info'></el-button>
        <span class="title">转入记录</span>
        <span class = "total">转入总金额：{{total}}</span> 
      </el-col>  
       <div align="center">
        <span>玩家ID</span>
      ...
      <el-table :data="list" style="width: 100%" :default-sort = "{prop: 'date', order: 'descending'}" align="center">
        <el-table-column prop="corTime" label="日期"  width="150" align="center"></el-table-column>
        <el-table-column prop="fromName" label="昵称"  width="180" align="center"></el-table-column>
        ...
        <el-table-column prop="transferMoney" label="转账数"  width="180" align="center" ></el-table-column>
      </el-table>  
...
export default {
  ...
  mounted(){
    myDispatch(this.$store, "UpPointLog",{  count:this.count,type: 1, page: this.page }).then((result) => {
      if(result.code==200){
        let logs = result.msg.logs;
        this.list=[];
        this.totalCount = result.msg.totalCount;
        if (!logs || logs.length === 0) {
          this.$message({type: "error",message: "没有转账记录!"});
          this.list=[];
          return;
        }
        logs.forEach((e, i) => {
      ...
      let match = {searchUid: this.searchUid,type: 1 };
      if(this.startTime){
  ...
  methods:{
    loadMore(){
      ...
      let page = this.page;
      let match = {count:this.count,searchUid: this.searchUid, type: 1, page: page };
      if(this.startTime){
        match.startTime = this.startTime;
      }
      if(this.endTime){
        ...
        myDispatch(this.$store, "UpPointLog",match).then((result) => {
          if(result.code == 200){
            let logs = result.msg.logs;
            this.list=[];
            this.totalCount = result.msg.totalCount;
    ...
    searchDate(){
      ...
      let match = {count:this.count, searchUid: this.searchUid,type: 1, page: this.page };
      if(this.startTime){
        match.startTime = this.startTime;
      }
      if(this.endTime){
